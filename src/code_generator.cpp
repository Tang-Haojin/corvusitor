...
bool CodeGenerator::load_data() {
  ...
  // Automatically discover module names from modules_dir_
  std::vector<std::string> module_names;

  DIR* dir = opendir(modules_dir_.c_str());
  if (!dir) {
    std::cerr << "Failed to open modules directory: " << modules_dir_ << std::endl;
    return false;
  }

  std::cout << "  Scanning directory: " << modules_dir_ << std::endl;

  struct dirent* entry;
  std::regex dir_pattern("verilator-compile-(.+)");
  while ((entry = readdir(dir)) != nullptr) {
    ...
    if (std::regex_match(entry_name, match, dir_pattern)) {
      ...
      std::string full_path = modules_dir_ + "/" + entry_name;
      struct stat st;
  ...
  // Parse each module header
  std::vector<ModuleInfo> modules_list;
  ModuleParser parser;
  for (const auto& name : module_names) {
    std::string header_path = modules_dir_ + "/verilator-compile-" + name + "/V" + name + ".h";

    std::cout << "  Parsing " << header_path << "..." << std::endl;
    ModuleInfo info = parser.parse(header_path);

    if (info.ports.empty()) {
      std::cerr << "Failed to parse module: " << name << std::endl;
      return false;
    }
...
std::string CodeGenerator::generate_assignment(const PortConnection& conn) {
  ...
  // Check if VlWide type
  bool is_vlwide = (driver_port->width_type == PortWidthType::VL_W);
...
bool CodeGenerator::generate_all(const std::string& output_file_base) {
  ...
  out_h << "// YuQuan Multi-Module Wrapper - Standalone Header\n";
  ...
  out_h << "\n";
  out_h << "// ============================================================================\n";
  out_h << "// VCorvusTopWrapper - Main wrapper class for YuQuan multi-module design\n";
  out_h << "// ============================================================================\n";
  ...
  out_cpp << "// YuQuan Multi-Module Wrapper Connection Code (Implementation)\n";
  out_cpp << "// Generated: " << __DATE__ << " " << __TIME__ << "\n";
  out_cpp << "// ============================================================================\n";
  out_cpp << "\n";
  out_cpp << "#include \"VCorvusTopWrapper_generated.h\"\n";
  out_cpp << "\n";
  out_cpp << "// ============================================================================\n";
