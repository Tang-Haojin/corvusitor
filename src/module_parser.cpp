...
ModuleParser::ModuleParser() {
  ...
  // Improved regex to handle possible spaces and special characters
...
ModuleInfo ModuleParser::parse(const std::string& header_path) {
  ModuleInfo info;
...
PortInfo ModuleParser::parse_port_macro(const std::string& line) {
  std::smatch match;
  PortInfo port;
...
std::string ModuleParser::generate_instance_name(const std::string& class_name, ModuleType type) {
  ...
  if (corvus_pos == 0) {
    name = name.substr(7);  // "corvus_" 长度为 7
  }
...
std::string ModuleParser::infer_lib_path(const std::string& header_path, const std::string& class_name) {
